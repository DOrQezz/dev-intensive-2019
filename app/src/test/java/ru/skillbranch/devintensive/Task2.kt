...
class Task2 {
    ...
    fun `Задание 2, проверка метода formatMessage`() {
        ...
        val message1 = TextMessage("0", user,
            Chat("0"), false, Date(), text1)
        val expected1 = "$firstName отправил сообщение \"$text1\""

        val message2 = TextMessage("0", user,
            Chat("0"), true, Date(), text2)
        val expected2 = "$firstName получил сообщение \"$text2\""

        val message3 = ImageMessage("0", user,
            Chat("0"), false, Date(), image1)
        val expected3 = "$firstName отправил изображение \"$image1\""

        val message4 = ImageMessage("0", user,
            Chat("0"), true, Date(), image2)
